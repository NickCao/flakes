...
let
  ...
  mode = pkgs.writeShellScriptBin "mode" ''
    ECREG=`sudo ${pkgs.xxd}/bin/xxd -s 0x1d -l 1 -ps /sys/kernel/debug/ec/ec0/io`
    echo -n "Fan Control: "
    case $ECREG in
      00)
        echo "Balanced Mode";;
    ...
    echo "Battery:"
    ${pkgs.upower}/bin/upower -i /org/freedesktop/UPower/devices/battery_BAT0 | ${pkgs.gnugrep}/bin/grep -E 'state|percentage|time to empty|energy-rate'

    ${pkgs.systemd}/lib/systemd/systemd-boot-check-no-failures
  '';
in
{
