  ...
  services = {
    ...
    udev = {
      ...
      extraRules = ''
        ...
        # hack for gpg agent issue
        ACTION=="add", SUBSYSTEM=="usb", ATTRS{idVendor}=="1050", ATTRS{idProduct}=="0406", RUN+="${pkgs.sudo}/bin/sudo -u nickcao ${pkgs.gnupg}/bin/gpg --card-status"
      '';
    };
    xserver = {
      enable = true;
